...
**Architecture:** This is a breaking OTX schema and signing-rule migration, not a long-term parallel extension. The existing `WitnessLayout::Otx` union id remains, but the `Otx` table replaces old `append_*_cells` and `seals` with `append_segments` and `base_seals`. The old overall `OtxAppend` hash path is removed after segmented layout, hash, planner, and tests are migrated.
...
- Add `Otx.base_seals: SealPairVec`.
- Move append seals into each `OtxAppendSegment.seals`.
- Remove long-term use of `SealScope::Append`.
...
- `crates/cobuild-core/src/protocol.rs`
  - Add segment flags; remove append seal scope use after migration.
...
- `crates/cobuild-core/src/seal.rs`
  - Add segment seal lookup and remove append-scope lookup usage.
...
fn generated_segmented_append_types_compile() {
    ...
    let _ = core::any::type_name::<lazy_reader::core::SegmentSealPairVec>();
    ...
    let _ = core::any::type_name::<entity::core::SegmentSealPairVec>();
}
```
...
In `crates/cobuild-types/schemas/core.mol`, add after `SealPairVec`:
...
In `otx_data`, parse `append_segments()` into `Vec<OtxAppendSegmentView>` and parse `base_seals()` into `Vec<SealPairView>`. Add:
...
- [ ] **Step 3: Add `otx_append_segment_hash`**
...
Add `write_segment_append_scope`, `write_segment_input_cells`, `write_segment_output_cells`,
...
let segment_view = otx
    ...
    .get(segment_index)
    .ok_or(CoreError::InvalidOtxLayout)?;
let seal = crate::seal::unique_segment_seal(self.lock_script_hash, &segment_view.seals)?;
...
```bash
rg -n "append_inputs:|append_outputs:|append_cell_deps:|append_header_deps:|OtxAppend|SealScope::Append|otx_append_hash|otx_append\\(" tests crates
```

Update every `OtxSegment { append_* }` fixture to use `append_segments`.

- [ ] **Step 2: Remove `SealScope::Append`**

After fixtures and engine no longer use append scope seals, modify `crates/cobuild-core/src/protocol.rs`:
...
and update `TryFrom<u8>` to accept only `0`.
...
```bash
rg -n "OtxAppend|otx_append_hash|append_input_cells|append_output_cells|append_cell_deps|append_header_deps|SealScope::Append" crates tests
```

Expected after edits: no matches except historical design docs and this plan.
...
- Removing `SealScope::Append` should happen after engine and fixtures stop using it.
