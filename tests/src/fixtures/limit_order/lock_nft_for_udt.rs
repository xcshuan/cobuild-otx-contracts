...
pub fn limit_order_lock_otx_with_sighash_all_fill_case() -> (CobuildTestFixture, TransactionView) {
    ...
    let nft_payload = nft_data(b"lock-sighash-fill", [1, 2, 3, 4], 1_717_171_719);
    let nft_input = live_input(
        fixture.context_mut(),
        typed_output(order_lock, nft.script.clone(), 100_000_000_000),
        nft_payload.clone(),
    );
    ...
    let fill_order_message = fixture
        ...
        .action_data(fill_action_data(1, script_hash(&buyer_lock)))
        .build();
    let otx = fixture
        .otx()
        .base_input_cells(1)
